...
evolve rules pots generations =
  let
    ...
    newPots = Set.fromList $ filter (nextPotState rules pots) [min..max]
  in if generations == 1 then newPots else evolve rules newPots (generations - 1)
...
main = do
  ...
  let
    ...
    finalState = evolve rules initialState 20
